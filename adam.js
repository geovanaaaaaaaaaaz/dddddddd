...
function createBot({ username, password }, botIndex) {
  ...
  function detectAndFollowMaster(bot, username) {
    ...
    setInterval(() => {
      // Only follow if none of the temporary walk/freeze modes are active
      if (!followMaster || combatWalkMode || masterHurtWalk || isFrozen) return;

      const master = bot.players[masterUsername]?.entity;
      
      // If the bot is falling into the void, make it jump
  ...
  function randomPlayerActions(bot) {
    setInterval(() => {
      // Don't perform random actions if in a special mode
      if (combatWalkMode || masterHurtWalk || isFrozen) return;
  ...
  function blockDetectionAndJump(bot) {
    ...
    setInterval(() => {
      // Don't perform block detection if in a special mode
      if (combatWalkMode || masterHurtWalk || isFrozen) return;
  ...
  bot.on('message', (message) => {
    ...
    // Check for enemy kill message to trigger a combat walk
    if (messageText.includes('Your enemy') && messageText.includes('still had') && messageText.includes('hearts remaining')) {
    ...
    // Respond to master's chat messages by going to hub
  ...
  bot.on('entityHurt', (hurtEntity) => {
    ...
    // Only react if the bot is the one hurt
    if (hurtEntity.username === bot.username && (currentTime - lastMasterHurtTime > masterHurtCooldown)) {
      // If low HP → unfreeze
      if (bot.health <= 4) { // Changed from < 2 to <= 4 (2 hearts)
    ...
    // Original behavior: Start a temporary walk when hit by master (if not frozen)
    if (hurtEntity === bot.entity && !isFrozen) {
